export function Impact() {
  return (
    <section className="bg-slate-950 px-4 py-12 text-white sm:px-6 md:py-24 lg:px-8">
      <div className="mx-auto max-w-4xl text-center">
        <p className="mb-3 text-xs font-black uppercase tracking-[0.18em] text-voices-purple md:text-sm">
          Impact
        </p>
        <h2 className="text-2xl font-black leading-tight sm:text-4xl md:text-5xl">
          Every booking pays someone.
        </h2>
        <p className="mx-auto mt-6 max-w-3xl text-lg font-semibold leading-relaxed text-slate-200 md:text-xl">
          Voices exists to get people paid for creative work. Our workshop
          programmes have taken people with no experience to their first paid
          club set, and our alumni have gone on to RINSE, NTS, KISS FM and Radio
          1 and BBC Radio 1Xtra. Commercial programming is what funds it. Every
          calendar we run expands the paid work available across the community
          we come from.
        </p>
      </div>
    </section>
  );
}
