...
import { voicesLiveStations, type VoicesLiveStationId } from "@/lib/voices/config";

export default function ScheduleList() {
  const [index, indexSet] = useState(0);
  const [station, setStation] = useState<VoicesLiveStationId>("kx");
  ...
  if (data) {
    ...
    const [date, shows] = entries[index] ?? entries[0] ?? [new Date().toISOString(), []];

    return (
      <div className="mx-auto w-full max-w-3xl px-3 pb-8 pt-4 md:px-6 md:pb-10 md:pt-6">
        <div className="sticky top-3 z-10 mx-auto mb-3 flex w-fit rounded-full border border-white/15 bg-black/40 p-1 shadow-lg shadow-black/20 backdrop-blur-xl">
